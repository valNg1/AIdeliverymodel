...
The delivery sequence: **Implementation → Commit → AI reads the diff → AI generates the test book → Automated / AI / Human validation → Push → Living Spec update.** Test evidence must reflect what was actually built, not only what was intended — see the [test book](compostelle-reuse-feedback/07-test-book.md), generated by reading commit `bc32314` and executing the suite.
