# Decision Log — Template

> Copy per product. Records every decision made during Discovery and beyond, including deferred decisions. Referenced throughout the [lifecycle](../01-operating-model/lifecycle.md).

**Product / Need:** ______________  **Owner of this log:** ______________

## Decisions

| # | Date | Decision | Rationale | Alternatives considered | Decided by | Status | Review date |
|---|---|---|---|---|---|---|---|
| 1 |  |  |  |  |  | Decided / Deferred |  |
| 2 |  |  |  |  |  | Decided / Deferred |  |

## Deferred decisions / open questions

| # | Question | Owner | Target date | Notes |
|---|---|---|---|---|
| 1 |  |  |  |  |

## Notes

- Every decision maps to a named human (**AI produces analyses; humans decide**).
- Deferred decisions must have an owner and a target date.
- Significant decisions should be reflected in the [AI Impact Canvas](ai-impact-canvas.md).
